...
%AFAZER: descrever as colunas das tabelas abaixo

\begin{table}
    \caption{Palavras/Tokens/Artigos por Documento}
    \centering
    \begin{tabular}{ c c c c }
        \hline
        \textbf{Doc.} & \textbf{num palavras} & \textbf{num tokens} & \textbf{num artigos} \\  
        \hline
        1 & 15228 & 23079 & 245 \\
        2 & 43579 & 65850 & 376 \\
        3 & 651 & 1197 & 7 \\
...
\begin{table}
    \caption{Quantitativos por Artigo}
    \centering
...
Os documentos descritos nas Tabelas \ref{tab:descricao-documentos-1} e \ref{tab:descricao-documentos-2} -- identificados na primeira coluna -- são os seguintes:

\begin{enumerate}
    \item Lei Complementar Estadual Nº 122/1994
    \item Resolução Nº 031/2021 – ALRN
    \item Resolução Nº 075/2024 – ALRN
    \item Resolução Nº 078/2024 – ALRN
    \item Resolução Nº 106/2018 – ALRN
    \item Resolução Nº 2388/2019 – ALRN
    \item Resolução Nº 077/2024 – ALRN
    \item Resolução Nº 089/2017 – ALRN
    \item Resolução Nº 618/2022 – ALRN
    \item Resolução Nº 64/2022 – ALRN
\end{enumerate}
...
A configuração do Llama, visto que foi feita a opção pelo uso do Ollama como meio de simplificar o processo, se deu de forma direta. Os arquivos de instalação do Ollama foram obtidos e executados diretamente da sua página de download. \cite{Ollama:2} Após a instalação, foi realizada a inclusão do modelo Llama em sua versão 3.1 (versão de 8B parâmetros), propriamente dito, tornando-o disponível para utilização.

O consumo da geração do modelo fica disponível por meio de uma API REST, a qual se encarrega de fazer o gerenciamento dos recursos disponíveis para a otimização do conteúdo. As requisições feitas à API apontam o modelo específico a ser utilizado (o Llama3.1, no caso deste estudo), o \textit{prompt} a ser utilizado pelo modelo, junto com um contexto (uma representação em \textit{embeddings} das interações anteriores com o modelo), também indicando se a resposta deve ser enviada \textit{token} a \textit{token}, à medida que é gerada, ou de forma inteiriça, com todo o conteúdo gerado sendo disponibilizado quando do término de sua produção pelo \ac{LLM}.

De forma a direcionar o comportamento do Llama na geração das respostas, cada requisição feita à API utiliza um modelo de \textit{prompt} com uma estrutura e informações fixas, alterando somente a pergunta feita e o teor dos documentos em que a resposta deve se basear. O modelo é dividido em quatro seções:
...
Além de incluir a pergunta, a resposta e o conjunto de documentos que foram utilizados na composição do \textit{prompt} enviado ao \ac{LLM}, a API de Gerenciamento de Fluxo coleta dados a respeito do processo como um todo (tempo levado na execução de cada etapa, métricas utilizadas, dentre outros) e os inclui na resposta devolvida ao usuário. Também cabe a esse elemento garantir persistência de dados sobre cada uma das interações realizadas, permitindo que, a posteriori, sejam analisados e utilizados para refinamento do processo e da estrutura proposta.

\section{Interface de Usuário}
\label{sec:Metodologia.4}

Dado o objetivo deste estudo, foi definida uma página \textit{web} como interface de usuário -- considerando sua simplicidade de desenvolvimento e implementação -- nos moldes de uma aplicação de envio/recebimento de mensagens de texto, como é o mais comum ao se desenhar uma ferramenta de \textit{chatbot}. A opção foi feita por manter uma interface limpa e minimalista, restringindo-se somente ao campo de envio de mensagens do usuário, bem como uma área superior com a exibição das mensagens enviadas e recebidas.

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{Imagens/interface_usuario.png}
    \caption{Interface de Usuário do Assistente de Busca}
...
\begin{table}
    ...
    \begin{tabular}{ l p{10cm} }
         ...
         Pergunta & Pergunta gerada pelo Llama com base no fragmento \\
         Recorte & Trecho do fragmento que o Llama identificou como resposta curta \\
