...
Portanto, é igualmente crucial mitigar a fragmentação e dispersão do contexto dos dados, evitando que informações correlacionadas -- e contíguas, no texto original -- se encontrem espalhadas ao longo de diversos fragmentos ao final do processo de divisão dos documentos em pedaços menores. Para isso é importante compreender que o sentido, o significado e a semântica comunicados residem não somente o conteúdo textual, nas palavras expressas do texto, senão também no que se infere a partir do que os precedeu, seja a nível textual ou estrutural.
...
\begin{table}[H]
    ...
    \textbf{**Nota}: O Manual de Processo Legislativo não é articulado (ver Seção \ref{subsubsec:Metodologia.1.1.4})
...
Dessa forma, mantivemos incorporados ao conteúdo textual da seção indicadores explícitos que sinalizam a posição do fragmento dentro da estrutura geral do documento, aplicando algum grau de redução da dispersão de informação e perda de dados contextuais.
...
O modelo \textit{text-embedding-3-large} apresenta desempenho substancialmente superior ao \textit{text-embedding-ada-002}, com destaque para os ganhos observados nos benchmarks MIRACL (de 31,4\% para 54,9\%) e MTEB (de 61,0\% para 64,6\%), indicando avanços consistentes na qualidade dos \textit{embeddings} em tarefas de recuperação de informações, similaridade semântica, classificação e ranqueamento.
...
Modelos de geração de \textit{embeddings} possuem restrições quanto ao número máximo de \textit{tokens} os quais são capazes de processar por cada entrada de texto a ser vetorizado, uma limitação que afeta diretamente o volume de texto sobre o qual o modelo pode ser aplicado. Conhecer a capacidade de cada modelo é essencial para definir estratégias de segmentação de documentos, especialmente diante da possibilidade de tratamento de textos longos.
...
\begin{table}[H]
    ...
    \begin{tabular}{ l c c }
        \hline
        \textbf{Modelo} & \textbf{Num. Max \textit{Tokens}} & \textbf{Tamanho do Vetor} \\  
        \hline
        \textit{GTE} & 512 & 768 \\
        \textit{BGE-m3} & 512 & 1.024 \\
        \textit{instructor-xl} & 512 & 768 \\
        \textit{BERT-QA} & 512 & 768\\
        \textit{Llama3.1} & $\geq 8192$ & 4.096 \\
        \textit{DeepSeek-r1:7b} & $\approx 8192$ & 3.584 \\
...
Como uma forma de realizar uma exploração inicial da qualidade dos \textit{embeddings} gerados por cada um dos modelos, buscamos utilizá-los para gerar representações vetoriais de um conjunto de documentos textuais de naturezas variadas, cujos \textit{embeddings} nos servirão como recurso para realizar as avaliações.
...
Os documentos passaram por uma fragmentação simples, sendo divididos em conjuntos de orações completas, os quais se limitaram ao comprimento máximo de 500 palavras, após o que foram vetorizados utilizando cada um dos modelos de geração de \textit{embeddings} apresentados na Seção \ref{subsec:Metodologia.2.1}, sendo o resultado armazenado para realização das análises a que nos propomos. Essa exploração consiste em utilizar os \textit{embeddings} para (1) utilizar como dados de treinamento para um classificador simples -- de forma a considerar a eficiência dos modelos em capturar as especificidades semânticas de cada um dos temas abordados pelos documentos; e (2) realizar particionamento do conjunto de dados em \textit{clusters} -- como forma de avaliar o quão bem cada modelo consegue capturar a similitude/discrepância entre fragmentos de documentos abordando temas diversos.

Adicionalmente, realizaremos a comparação dos \textit{embeddings} gerados em dois contextos:

\begin{itemize}
    \item \textbf{Dataset 1} — análise de um conjunto de documentos com \textit{estilos, formas e conteúdos distintos}, utilizando os arquivos: \texttt{apocrifo.txt}, \texttt{cabala.txt}, \texttt{comunismo.txt}, \texttt{culinaria.txt}, \texttt{fisica.txt}, \texttt{historia.txt} e \texttt{regimento\_al.txt};
    \item \textbf{Dataset 2} — justaposição de documentos com \textit{estilos e formas semelhantes, porém com conteúdo diverso}, utilizando os arquivos: \texttt{regimento\_al.txt} e \texttt{cod\_penal.txt}.
\end{itemize}

Com o objetivo de obter uma intuição visual mais clara acerca da similaridade ou dissimilaridade entre os arquivos utilizados em cada conjunto de dados, realizamos uma representação gráfica dos \textit{embeddings} gerados por cada modelo. Para isso, aplicamos uma técnica de redução de dimensionalidade, uma vez que os \textit{embeddings} operam em espaços vetoriais de alta dimensão, o que inviabiliza sua visualização direta. A técnica escolhida foi \textit{\ac{TSNE}}, que preserva, de forma eficaz, tanto a estrutura local quanto a estrutura global dos dados em espaços de menor dimensão. Assim, ao projetarmos os vetores em um espaço bidimensional, conseguimos observar visualmente como os documentos se agrupam ou se separam com base em suas características semânticas. Essa representação imagética permite uma análise exploratória complementar, possibilitando identificar padrões de proximidade, dispersão ou sobreposição entre os documentos. As Figuras~\ref{fig:embeddings-1} e~\ref{fig:embeddings-2} ilustram, respectivamente, os resultados obtidos para o Dataset~1 e o Dataset~2, evidenciando as distinções na organização espacial dos conteúdos analisados.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.74\linewidth]{Imagens/embeddings_1.png}
    \caption{Representação em baixa dimensão do dataset 1}
    Fonte: autoria própria
...
\begin{figure}[H]
    ...
    \caption{Representação em baixa dimensão do dataset 2}
    Fonte: autoria própria
    \label{fig:embeddings-2}
\end{figure}


Para a avaliar o comportamento dos modelos sobre dados com estilos distintos com classificação, foi empregado o algoritmo \textit{Random Forest} -- por meio da classe \texttt{RandomForestClassifier} disponível na biblioteca \texttt{sklearn.ensemble}. Após o treinamento, a avaliação do desempenho preditivo do classificador foi realizada utilizando duas métricas: acurácia e coeficiente de correlação de Matthews (MCC).

A acurácia é definida como a proporção de previsões corretas (tanto de positivos quanto de negativos) em relação ao total de exemplos avaliados, sendo uma métrica intuitiva e amplamente utilizada, especialmente em cenários com classes balanceadas. No entanto, como sua interpretação pode ser enganosa em contextos em que há uma disparidade muito grande entre os números de instâncias em cada classe, também foi utilizada, como forma de contornar essa limitação, o \ac{MCC}, uma medida mais robusta de qualidade do modelo, especialmente recomendada em cenários com distribuições desbalanceadas de classes. O \ac{MCC} leva em consideração todas as quatro componentes da matriz de confusão (verdadeiros positivos, verdadeiros negativos, falsos positivos e falsos negativos), e produz um valor entre –1 e +1, de forma que +1 indica uma predição perfeita, 0 indica desempenho equivalente ao acaso e –1 indica uma total discordância entre predições e rótulos verdadeiros.

A inclusão do \ac{MCC} na avaliação permite uma análise mais abrangente e confiável do desempenho do modelo, complementando os valores expressos pela acurácia.

A Tabela \ref{tab:scores-classificacao-1} apresenta os resultados da classificação feita sobre Dataset~1, composto por documentos com estilos, formas e conteúdos distintos, ao passo que os resultados da classificação feita sobre o Dataset~2, cujo conteúdo é dois documentos que diferem somente no conteúdo, estão dispostos na Tabela \ref{tab:scores-classificacao-1}.

\begin{table}[H]
    \centering    
    \begin{minipage}{0.48\textwidth}
        \caption{Resultados do Dataset 1}
        \centering
        \begin{tabular}{ l c c }
            \hline
            \textbf{Modelo} & \textbf{Acurácia} & \textbf{MCC} \\  
            \hline
            \textit{GTE} & 1,00 & 1,00 \\
            \textit{BGE-m3}  & 1,00 & 1,00 \\
            \textit{instructor-xl} & 1,00 & 1,00 \\
            \textit{BERT-QA} & 0,98 & 0,98 \\
            \textit{Llama3.1} & 1,00 & 1,00 \\
            \textit{DeepSeek-r1}  & 0,94 & 0,92 \\
            \textit{ada-002} (OpenAI) & 1,00 & 1,00 \\
            \textit{3-large} (OpenAI) & 1,00 & 1,00 \\
            \hline
        \end{tabular}           
        \vspace{2mm}
    ...
    \hfill    
    \begin{minipage}{0.48\textwidth}
        \caption{Resultados do Dataset 2}
        \centering
        \begin{tabular}{ l c c }
            \hline
            \textbf{Modelo} & \textbf{Acurácia} & \textbf{MCC} \\  
            \hline
            \textit{GTE} & 0,99 & 0,97 \\
            \textit{BGE-m3} & 0,99 & 0,99 \\
            \textit{instructor-xl} & 0,99 & 0,99 \\
            \textit{BERT-QA} & 0,99 & 0,99 \\
            \textit{Llama3.1} & 0,99 & 0,99 \\
            \textit{DeepSeek-r1 }& 0,94 & 0,89 \\
            \textit{ada-002} (OpenAI) & 0,99 & 0,99 \\
            \textit{3-large} (OpenAI) & 1,00 & 1,00 \\
            \hline
        \end{tabular}       
        \vspace{2mm}
...
Os resultados obtidos tanto para o Dataset~1 quanto para o Dataset~2 indicam capacidade preditiva equivalente entre os modelos avaliados, com exceção do \textit{DeepSeek-r1}, que apresentou desempenho significativamente inferior em comparação aos demais.




De forma a avaliar o comportamento dos modelos sobre textos com mesmo estilo, mas conteúdo diverso usando \textit{clustering} sobre as representações vetoriais geradas, foram utilizados os algoritmos de particionamento \textit{K-Means}, implementado pela função \texttt{KMeans} do módulo \texttt{sklearn.cluster}, e \textit{DBScan}, com implementação por \texttt{sklearn.cluster.DBSCAN}. Neste experimento, como aprioristicamente sabemos a quantidade de partições (documentos com temas bem definidos) foi fixado o valor de \textit{k} em \textit{7} para o Dataset~1 e \textit{2} para o Dataset~2.
...
As Tabelas \ref{tab:kmeans-dataset1-sem-reducao} a \ref{tab:dbscan-dataset2-sem-reducao} apresentam os resultados da aplicação tanto do \textit{K-Means} quanto do \textit{DBScan} sobre as representações vetoriais geradas pelos modelos de \textit{embeddings} sendo avaliados, sem fazer neles quaisquer tipos de alteração. Assim sendo, são vetores de alta dimensionalidade, variando de acordo com o modelo (\textit{GTE}: 768 dimensões, \textit{BGE-m3}: 1.024 dimensões, \textit{instructor-xl}: 768 dimensões, \textit{BERT-QA}: 768 dimensões, \textit{Llama3.1}: 4.096 dimensões, \textit{DeepSeek-r1}: 3.584 dimensões, \textit{ada-002}: 1.536 dimensões e \textit{3-large}: 3.072 dimensões.

Quanto às As Tabelas \ref{tab:kmeans-dataset1-com-reducao} a \ref{tab:dbscan-dataset2-com-reducao}, nelas são expostos os resultados da aplicação dos algoritmos de particionamento sobre a versão com dimensionalidade reduzida de cada conjunto de \textit{embeddings}, com \textit{3} dimensões, geradas pelo uso de \ac{TSNE}, as quais foram utilizadas para gerar as Figuras \ref{fig:embeddings-1} e \ref{fig:embeddings-2}, já apresentadas. 

\begin{table}
    \centering
    % Primeira linha
    \begin{minipage}{0.48\textwidth}
        \caption{Kmeans -- Dataset 1}
        \centering
        \begin{tabular}{ l c c }
            \hline
        ...
        \vspace{2mm}
        \small Fonte: O autor (2025)
        \label{tab:kmeans-dataset1-sem-reducao}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \caption{Kmeans -- Dataset 2}
        \centering
        \begin{tabular}{ l c c }
            \hline
            \textbf{Modelo} & \textbf{Silhouette} & \textbf{DB} \\  
        ...
        \vspace{2mm}
        \small Fonte: O autor (2025)
        \label{tab:kmeans-dataset2-sem-reducao}
    \end{minipage}
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \vspace{2mm}
        \small Fonte: O autor (2025)
        \label{tab:dbscan-dataset1-sem-reducao}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \caption{DBScan -- Dataset 2}
        \centering
        \begin{tabular}{ l c c }
            \hline
            \textbf{Modelo} & \textbf{Silhouette} & \textbf{DB} \\  
        ...
        \vspace{2mm}
        \small Fonte: O autor (2025)
        \label{tab:dbscan-dataset2-sem-reducao}
    \end{minipage}
...
\begin{table}
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \vspace{2mm}
        \small Fonte: O autor (2025)
        \label{tab:kmeans-dataset1-com-reducao}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \caption{Kmeans -- Dataset 2 -- 3D}
        \centering
        \begin{tabular}{ l c c }
            \hline
            \textbf{Modelo} & \textbf{Silhouette} & \textbf{DB} \\  
        ...
        \vspace{2mm}
        \small Fonte: O autor (2025)
        \label{tab:kmeans-dataset2-com-reducao}
    \end{minipage}
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \vspace{2mm}
        \small Fonte: O autor (2025)
        \label{tab:dbscan-dataset1-com-reducao}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \caption{DBScan -- Dataset 2 -- 3D}
        \centering
        \begin{tabular}{ l c c }
            \hline
            \textbf{Modelo} & \textbf{Silhouette} & \textbf{DB} \\  
        ...
        \vspace{2mm}
        \small Fonte: O autor (2025)
        \label{tab:dbscan-dataset2-com-reducao}
    \end{minipage}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{ l l }
        ...
        \hline
        documentos\_rh\_bge\_m3 & BAAI/BGE-m3 \\
        documentos\_rh\_openai & text-embedding-ada-002 \\
        documentos\_rh\_alibaba & Alibaba-NLP/gte-multilingual-base \\
        documentos\_rh\_instructor & hkunlp/instructor-xl \\
        documentos\_rh\_bert\_pt & pierreguillou/bert-base-cased-squad-v1.1-portuguese \\
        documentos\_rh\_deepseek & deepseek-r1:7b \\
        documentos\_rh\_llama & Llama3.1 \\
        \hline
    \end{tabular}   
...
De forma a direcionar o comportamento dos \ac{LLM}s na geração das respostas, cada requisição feita à API utiliza uma mensagem de sistema fixa, bem como um modelo de \textit{prompt} com uma estrutura pré-definida, alterando somente a pergunta feita e o teor dos documentos em que a resposta deve se basear. Assim, tal como é visível na Figura \ref{fig:pilha-mensagem-ollama}, os dados enviados à API têm a seguinte estrutura:
...
\begin{itemize}
    ...
    \begin{quote}
    {
        ...
        Você é um assistente que responde a dúvidas de servidores da ALRN sobre o regimento interno da ALRN, o regime jurídico dos servidores estaduais do RN, bem como resoluções da ALRN.
