...
\begin{enumerate}
    ...
    \item API Gerenciadora: comunica-se com a interface e faz a integração com o banco de vetores e com a ferramenta de execução do \ac{LLM};
    \item Banco de Vetores: armazena os documentos base da aplicação e permite consultas por similaridade semântica;
    \item Ferramenta de Execução de \ac{LLM}s: executa \ac{LLM}s, gerando respostas com base em \textit{prompts}.
\end{enumerate}

O fluxo de interações entre as partes componentes da aplicação se dá da seguinte forma:
...
Após alguma exploração preliminar, elencamos alguns modelos para comparação, avaliação de desempenho e definição final para uso. Os modelos que consideramos mais promissores são introduzidos a seguir. 
...
\begin{itemize}
    \item \textbf{llama3.1:latest}: modelo que será detalhado na Seção \ref{subsec:Assistente.2.1}, desenvolvido pela Meta AI. Trata-se de um \ac{LLM}, por natureza otimizado para geração de texto. Será utilizado por meio da ferramenta Ollama (Seção \ref{subsec:Assistente.2.3}).
...
No final de 2024, a empresa publicou um relatório técnico que apresentava seu modelo DeepSeek-V3, o qual, junto com a publicação de seu modelo seguinte, o DeepSeek-R1, surpreendeu o mercado, demonstrando a viabilidade de se produzir e um \ac{LLM} poderoso, competitivo e aberto por uma fração do custo de modelos fechados. O relatório demonstra que o DeepSeek-V3 incorpora uma arquitetura baseada na abordagem denominada \textit{Mixture-of-Experts} (MoE/DeepSeekMoE), que consiste dividir o modelo em diversas partes menores, os ``\textit{experts}'', que são especializadas em certos tipos de dados de entrada ou de tarefas, sendo utilizados pelo modelo somente quando necessário, aumentando a otimização de recursos. Com isso, embora sua estrutura total compreenda um vasto número de parâmetros, apenas uma parte deles é ativada simultaneamente durante o processamento de cada palavra, o que proporciona maior eficiência computacional e redução de custos operacionais.
